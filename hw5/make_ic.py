...
#modified from Gizmo make_IC.py example and with input from Tyler Neslon
...
def make_IC():
    ...
    # N_1D=22; # 1D particle number (so total particle number is N_1D^DIMS)
    # N_1D = int(10**(4/3))
    # output filename

    # Lbox = 10.0 # box side length
    ...
    #Nparticles = int(1e5) # np.round(2. * (1.*N_1D)**DIMS)
    rMax = 3000  # sim uses 1kpc as unit length
    ...
    mu = 2 * np.random.rand(Nparticles) - 1
    r = rMax * (np.random.rand(Nparticles)) ** (1 / 3)

    xyproj = r * np.sin(np.arccos(mu))
    x = xyproj * np.cos(phi)
    y = xyproj * np.sin(phi)
    z = r * mu

    # set the IDs: these must be unique, so we start from the maximum gas ID and go up
    id_p1 = np.arange(1, Nparticles + 1)
    ...
    mass = np.ones(Nparticles) * (M_Halo / Nparticles / U_Mass)  # simulation units are in 10**10 Msun
    ...
    #
    npart = np.array(
        [0, Nparticles, 0, 0, 0, 0])  # we have gas and particles we will set for type 3 here, zero for all others
    ...
    q = np.zeros((Nparticles, 3));
    q[:, 0] = x
    q[:, 1] = y
    q[:, 2] = z
    p.create_dataset("Coordinates", data=q)
    q = np.zeros((Nparticles, 3))  # q[:,0]=vx_d; q[:,1]=vy_d; q[:,2]=vz_d;
    p.create_dataset("Velocities", data=q)
    p.create_dataset("ParticleIDs", data=id_p1)
